...
功能做完之后，我用 `gh` CLI 做了所有 GitHub 相关的事：

```bash
# 更新仓库描述和标签
gh repo edit Kingson4Wu/GridSnap \
  --description "Split any photo into a grid for Instagram" \
  --add-topic pwa --add-topic instagram --add-topic react

# 发布 v1.0.0
gh release create v1.0.0 --title "v1.0.0 — Initial Release" --notes "..."

# 开启 Discussions，只保留 Squash merge，PR 合并后自动删分支
gh api --method PATCH repos/Kingson4Wu/GridSnap \
  --field has_discussions=true \
  --field allow_merge_commit=false \
  --field delete_branch_on_merge=true
```

Wiki 是让 AI 写好内容之后直接 push 进去的：Home、User Guide、Architecture、Contributing、Roadmap，五个页面。

我没开过一次 GitHub 网页（除了上传 Social Preview 图片，那个 API 不开放）。
